...
ll_evertest <- function(mod, fp, dat) {
  ...
  if (any(is.na(mu)) || any(mu < 0) || any(mu > 1)) {
    ...
    llk <- sum(stats::dbinom(x = dat$nsuccess, size = dat$neff, prob = mu, log = TRUE))
  }
...
art_constraint_penalty <- function(mod, fp, max_year = 2022) {
  ...
  penalty <- sum(stats::dnorm(x = 0, mean = ratio_late_per_1000_untreated,
                       sd = 20, log = TRUE))
  return(penalty)
}
...
## We calculate penalty for RR of males on the log(rate) scale (and use same SD as for females)
  ...
  n_k2 <- n_k1 * 2 - 10
    ...
    penalty_rr_m <- log(stats::plogis(theta[n_k2 + 2]) * 1.1) - log(stats::plogis(theta[n_k2 + 1]) * 1.1)
  ...
  lprior <-
    ...
    ## Relative testing among PLHIV diagnosed, untreated. 1.50 (95%CI: 0.14-6.00) # stats::plogis(stats::qlogis(1.5/8) - 1.96*1.31)*8
    stats::dnorm(x = theta[n_k2], mean = stats::qlogis(1.5/8), sd = 1.31, log = TRUE) +
    ## Prior for male RR 0.6 (95%CI: 0.07-1.05)# stats::plogis(stats::qlogis(0.6/1.1) + 1.96*1.46) * 1.1
    sum(stats::dnorm(x = theta[n_k2 + 1], mean = stats::qlogis(0.6/1.1), sd = 1.46, log = TRUE)) +
    ...
    ## Relative factor for re-testing among PLHIV. 1.00 (95%CI: 0.10-1.90) # 0.05 + stats::plogis(stats::qlogis(0.95 / 1.90) - 1.96*1.85) * (1.95 - 0.05)
    stats::dnorm(x = theta[n_k2 + 5], mean = stats::qlogis(0.95 / 1.90), sd = 1.85, log = TRUE) +
    ## Relative testing among PLHIV already on ART (95%CI: 0.01-0.90) # stats::plogis(stats::qlogis(0.25) - 1.96*1.68)
    ...
    ## RR OI diangosed for HIV relative to ART coverage 1.0 (0.3-1.7) # 0.25 + stats::plogis(stats::qlogis(0.5) - 1.96*1.75) * (1.75 - 0.25)
